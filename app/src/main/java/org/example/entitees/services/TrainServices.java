...
public class TrainServices {
    ...
    public void updateTrain(Train upadatedTrain){
        OptionalInt index= IntStream.range(0,trainList.size())
                .filter(i->trainList.get(i).getTrainId().equalsIgnoreCase(upadatedTrain.getTrainId()))
                .findFirst();
        if (index.isPresent()){
            trainList.set(index.getAsInt(),upadatedTrain);
            saveTrainListToFile();
        }
        else {
            addTrain(upadatedTrain);
        }
    }
